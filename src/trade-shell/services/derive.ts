/**
 * derive.ts — Enriquece TotemProfile (backend crudo) con campos UI.
 * ONCHAIN WINS: todo lo derivado es cosmético. price/supply/score intocables.
 */
import type { TotemProfile } from "../../lib/tradeApi";

export const GRADUATION_SUPPLY = 1_000_000;
...
export function deriveCurvePercent(supply: number): number {
  if (!supply || supply <= 0) return 0;
  return Math.min(100, (supply / GRADUATION_SUPPLY) * 100);
}
...
export function enrich(t: TotemProfile): Enriched {
  return {
    ...
    curvePercent: deriveCurvePercent(t.supply),
  };
}
