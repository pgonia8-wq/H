...
const STATE_LABELS: Record<string, string> = {
  ...
  LOCKDOWN: "Lockdown — Read-only feed, trading active, drain queue",
  STABILIZATION: "Stabilization — Feed cache-only, 70% DB write reduction, trading active",
  HARD_SAFE: "Hard Safe — Trading only, all social queued, waiting for recovery",
};
...
function GpiGauge({ gpi }: { gpi: number }) {
  const color = gpi >= 90 ? "#ff2020" : gpi >= 75 ? "#f05050" : gpi >= 50 ? "#f7a606" : "#10f090";
  const label = gpi >= 90 ? "LOCKDOWN" : gpi >= 75 ? "CRITICAL" : gpi >= 50 ? "STRESS" : "NORMAL";
  const pct = Math.min(100, Math.max(0, gpi));

  return (
    <div style={{ textAlign: "center" }}>
      <div style={{ fontSize: 11, color: "#666", fontWeight: 500, marginBottom: 6 }}>Global Pressure Index</div>
      <div style={{ position: "relative", width: 120, height: 120, margin: "0 auto" }}>
        ...
        <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center" }}>
          <div style={{ fontSize: 28, fontWeight: 800, color }}>{gpi}</div>
...
export default function InfraPanel({ apiCall }: InfraPanelProps) {
  ...
  const { system, metrics, queue, tracing, rateLimit: rl, circuitBreakers, uxHints } = data;
  const stateColor = STATE_COLORS[system.state] || "#666";
  const gpi = data.gpi || 0;
    ...
    <div>
        ...
        <div style={{ ...cardStyle, borderColor: stateColor + "40", background: stateColor + "08" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <div>
              ...
              {system.dbWriteReduction > 0 && (
                <div style={{ fontSize: 10, color: "#c060ff", marginTop: 2 }}>DB Write Reduction: {system.dbWriteReduction}%</div>
              )}
              {system.aggressiveBatching && (
                <div style={{ fontSize: 10, color: "#ff4060", marginTop: 2 }}>Aggressive Batching: ACTIVE</div>
              )}
            </div>
          </div>

          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 12 }}>
            {["NORMAL", "STRESS", "CRITICAL", "STABILIZATION", "LOCKDOWN", "HARD_SAFE"].map(s => (
              <button key={s} onClick={() => setConfirming(s)} disabled={system.state === s}
                style={{ ...btnStyle(STATE_COLORS[s] || "#666"), opacity: system.state === s ? 0.3 : 1, cursor: system.state === s ? "default" : "pointer", padding: "6px 10px", fontSize: 11 }}>
                {STATE_ICONS[s]} {s}
              </button>
            ))}
            <button onClick={() => doAction("enable_auto")} style={{ ...btnStyle("#10f090"), padding: "6px 10px", fontSize: 11 }}>
              🔄 Auto
            </button>
          </div>
          ...
          {confirming && (
            <div style={{ marginTop: 12, padding: 12, background: "#0a0a0f", borderRadius: 8, border: "1px solid #2a2a3e" }}>
              <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8, color: STATE_COLORS[confirming] || "#666" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => actionReason && doAction("force_state", { state: confirming, reason: actionReason })}
                  disabled={!actionReason} style={{ ...btnStyle(STATE_COLORS[confirming] || "#666"), opacity: actionReason ? 1 : 0.4 }}>Confirm</button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 12, marginBottom: 16 }}>
        <div style={cardStyle}>
          ...
          {system.cache.enabled && <div style={{ fontSize: 10, color: "#555" }}>Feed: {system.cache.feedTTL / 1000}s</div>}
        </div>
        <div style={cardStyle}>
          <div style={labelStyle}>Batch Writes</div>
          <div style={{ ...valueStyle, fontSize: 14, color: system.batchEnabled ? "#f7a606" : "#666" }}>{system.batchEnabled ? "ON" : "OFF"}</div>
        </div>
        <div style={cardStyle}>
        ...
        <div style={cardStyle}>
          <div style={labelStyle}>DB Write Cut</div>
          <div style={{ ...valueStyle, fontSize: 14, color: system.dbWriteReduction > 0 ? "#c060ff" : "#666" }}>{system.dbWriteReduction}%</div>
        </div>
      </div>
      ...
      {circuitBreakers && (
        <div style={cardStyle}>
          <h3 style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, color: "#aaa" }}>⚡ Circuit Breakers</h3>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 10 }}>
            {Object.entries(circuitBreakers).map(([name, info]: [string, any]) => (
              <div key={name} style={{ background: "#0a0a12", border: "1px solid #1e1e2e", borderRadius: 8, padding: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#e0e0e0" }}>{name}</span>
                  <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 4,
                    background: BREAKER_COLORS[info.state] + "20", color: BREAKER_COLORS[info.state] }}>
                    {info.state}
                  </span>
                </div>
                <div style={{ fontSize: 10, color: "#666" }}>
                  <div>Failures: {info.failures} | Trips: {info.totalTrips}</div>
                  {info.lastFailure && <div>Last fail: {new Date(info.lastFailure).toLocaleTimeString()}</div>}
                </div>
                {info.state !== "CLOSED" && (
                  <button onClick={() => doAction("reset_breaker", { subsystem: name })}
                    style={{ ...btnStyle("#10f090"), marginTop: 6, padding: "4px 8px", fontSize: 10 }}>Reset</button>
                )}
              </div>
            ))}
          </div>
      ...
      {queue?.backpressure && (
        <div style={cardStyle}>
          <h3 style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, color: "#aaa" }}>🌊 Queue Backpressure</h3>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
            <div style={{ fontSize: 12 }}>
              <span style={{ color: "#666" }}>Total Pending:</span>{" "}
              <span style={{ fontWeight: 700, color: queue.backpressure.totalPending > 50000 ? "#f05050" : "#e0e0e0" }}>
                {queue.backpressure.totalPending.toLocaleString()}
              </span>
            </div>
            <div style={{ fontSize: 12 }}>
              <span style={{ color: "#666" }}>Level:</span>{" "}
              <span style={{ fontWeight: 700, padding: "2px 8px", borderRadius: 4,
                background: queue.backpressure.level === "NORMAL" ? "#10f09020" : "#f0505020",
                color: queue.backpressure.level === "NORMAL" ? "#10f090" : "#f05050" }}>
                {queue.backpressure.level}
              </span>
            </div>
            {queue.worker?.dropped > 0 && (
              <div style={{ fontSize: 12 }}>
                <span style={{ color: "#666" }}>Dropped:</span>{" "}
                <span style={{ fontWeight: 700, color: "#f05050" }}>{queue.worker.dropped}</span>
              </div>
            )}
          </div>
          <div style={{ display: "flex", gap: 4, marginTop: 6 }}>
            {Object.entries(queue.backpressure.thresholds).map(([name, val]: [string, any]) => (
              <div key={name} style={{ flex: 1, textAlign: "center", fontSize: 9, padding: "4px 2px", borderRadius: 4,
                background: queue.backpressure.totalPending >= val ? "#f0505020" : "#1a1a2e",
                color: queue.backpressure.totalPending >= val ? "#f05050" : "#555",
                border: `1px solid ${queue.backpressure.totalPending >= val ? "#f0505040" : "#1e1e2e"}` }}>
                <div style={{ fontWeight: 600 }}>{name.replace("_", " ")}</div>
                <div>{(val as number).toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
        <div style={cardStyle}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 12 }}>
            ...
            <div><span style={{ color: "#666" }}>GPI:</span> <span style={{ fontWeight: 700, color: gpi >= 75 ? "#f05050" : gpi >= 50 ? "#f7a606" : "#10f090" }}>{gpi}/100</span></div>
          </div>
        </div>
        <div style={cardStyle}>
          ...
          {queue?.worker && (
            <div style={{ fontSize: 12 }}>
              ...
              <div><span style={{ color: "#666" }}>Processed:</span> {queue.worker.processed} | <span style={{ color: "#666" }}>Failed:</span> {queue.worker.failed} | <span style={{ color: "#666" }}>Dropped:</span> {queue.worker.dropped}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
        ...
        <div style={cardStyle}>
          <h3 style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, color: "#aaa" }}>🔍 Tracing & Log Tiering</h3>
          <div style={{ fontSize: 12 }}>
            <div><span style={{ color: "#666" }}>Buffer:</span> {tracing?.buffered || 0} / {tracing?.bufferSize || 50}</div>
            <div><span style={{ color: "#666" }}>Sample Rate:</span> <span style={{ fontWeight: 600, color: tracing?.sampleRate < 0.5 ? "#f7a606" : "#10f090" }}>{tracing?.samplePercent || "100%"}</span></div>
            <div><span style={{ color: "#666" }}>Total:</span> {tracing?.stats?.total || 0} | <span style={{ color: "#666" }}>Sampled:</span> {tracing?.stats?.sampled || 0} | <span style={{ color: "#f05050" }}>Dropped:</span> {tracing?.stats?.dropped || 0}</div>
            <div><span style={{ color: "#666" }}>Flushed:</span> {tracing?.stats?.flushed || 0} | <span style={{ color: "#666" }}>Errors:</span> {tracing?.stats?.errors || 0}</div>
      ...
      {/* UX Hints Preview */}
      {uxHints && uxHints.messages && uxHints.messages.length > 0 && (
        <div style={cardStyle}>
          <h3 style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, color: "#aaa" }}>💬 Active UX Messages (user-facing)</h3>
          {uxHints.messages.map((msg: any, i: number) => (
            <div key={i} style={{ padding: "6px 10px", marginBottom: 4, borderRadius: 6, fontSize: 12,
              background: msg.type === "danger" ? "#f0505015" : msg.type === "warning" ? "#f7a60615" : "#10f09015",
              color: msg.type === "danger" ? "#f05050" : msg.type === "warning" ? "#f7a606" : "#10f090",
              border: `1px solid ${msg.type === "danger" ? "#f0505030" : msg.type === "warning" ? "#f7a60630" : "#10f09030"}` }}>
              {msg.type === "danger" ? "🔴" : msg.type === "warning" ? "🟡" : "🟢"} {msg.text}
            </div>
          ))}
        </div>
      )}
          ...
          <div>
            <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
              ...
              <input type="text" placeholder="Trace ID" value={logFilter.traceId || ""} onChange={e => setLogFilter({ ...logFilter, traceId: e.target.value || undefined })}
                style={{ background: "#1a1a2e", border: "1px solid #2a2a3e", borderRadius: 6, color: "#e0e0e0", padding: "4px 8px", fontSize: 11, width: 160 }} />
              <button onClick={doQueryLogs} style={btnStyle("#6060ff")}>Search</button>
            </div>
            {logs.length > 0 ? (
        ...
        <div style={{ maxHeight: 200, overflowY: "auto" }}>
          {(data.stateTransitions || []).slice().reverse().map((t: any, i: number) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0", borderBottom: "1px solid #1a1a2a", fontSize: 11 }}>
              ...
              {t.gpi !== undefined && <span style={{ color: "#6060ff", fontSize: 9 }}>GPI:{t.gpi}</span>}
      ...
      {/* Config */}
      <div style={cardStyle}>
        <h3 style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, color: "#aaa" }}>⚙️ Auto-Scaling & Recovery Thresholds</h3>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 12 }}>
          ...
          <div><span style={{ color: "#666" }}>Stabilization After:</span> <span style={{ fontWeight: 600 }}>{system.config.stabilizationAfterCriticalSec}s critical</span></div>
          <div><span style={{ color: "#666" }}>Hard Safe After:</span> <span style={{ fontWeight: 600 }}>{system.config.hardSafeAfterLockdownSec}s lockdown</span></div>
          <div><span style={{ color: "#666" }}>Recovery Error Rate:</span> <span style={{ fontWeight: 600, color: "#10f090" }}>{"<"}{system.config.recoveryErrorRate}%</span></div>
          <div><span style={{ color: "#666" }}>Recovery P95:</span> <span style={{ fontWeight: 600, color: "#10f090" }}>{"<"}{system.config.recoveryLatencyP95}ms</span></div>
        </div>
      </div>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: "flex", gap: 8, fontSize: 11, flexWrap: "wrap" }}>
          {["1. Trading (BUY/SELL)", "2. Financial Integrity", "3. Token State", "4. Queue Processing", "5. Social Features", "6. UI Realtime"].map((p, i) => (
            <span key={i} style={{ background: "#1a1a2e", padding: "3px 8px", borderRadius: 6, color: i === 0 ? "#10f090" : i < 3 ? "#f7a606" : "#666", fontWeight: i === 0 ? 700 : 400 }}>{p}</span>
          ))}
        </div>
      </div>
    </div>
