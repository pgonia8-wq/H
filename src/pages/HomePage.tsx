...
const HomePage = (props: { userId: string | null }) => {
  ...
  const { wallet } = useMiniKitUser(); // fallback si no hay userId

  const containerRef = useRef<HTMLDivElement>(null);

  // ✅ Corregido: usamos currentUserId para todo
  const currentUserId = userId || wallet;
  ...
  return (
    ...
    >
      {/* Header, botones, modales, feed completo */}
      <header className={`sticky top-0 z-20 w-full px-4 py-3 flex items-center justify-between border-b ${theme === 'dark' ? 'border-white/10 bg-black/90' : 'border-black/10 bg-white/90'} backdrop-blur-xl`}>
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          <div className="relative cursor-pointer">
            <div className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center text-lg shadow-inner">🔔</div>
