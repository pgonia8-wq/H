...
const GlobalChatRoom: React.FC<GlobalChatRoomProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-md flex flex-col bg-gray-900 rounded-2xl shadow-2xl transform transition-transform duration-200 scale-95 animate-scale-up">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-800">
          ...
          {(showGoldChat ? goldMessages : messages).map((m) => (
            <div key={m.id || Math.random()} className={`flex ${m.sender_id === currentUserId ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[70%] px-4 py-2 rounded-xl ${m.sender_id === currentUserId ? "bg-purple-600 text-white" : "bg-gray-700 text-gray-200"}`}>
                <span className="text-xs opacity-70 block mb-1">{m.sender_id?.slice(0, 10)}...</span>
                {m.content}
              </div>
            </div>
          ))}
        ...
        {!goldSubscribed && showGoldChat && (
          <div className="flex flex-col items-center justify-center p-6 bg-gray-700 text-white animate-fade-in">
            ...
            <button onClick={handleGoldSubscribe} className="px-6 py-3 bg-yellow-500 text-black rounded-xl hover:bg-yellow-600 transition">
              Suscribirse a Gold
            </button>
          </div>
        )}
        ...
        {(!showGoldChat || goldSubscribed) && (
          <div className="flex gap-2 p-4 border-t border-gray-700 bg-gray-900 sticky bottom-0">
            <input
              type="text"
              className="flex-1 bg-gray-800 px-4 py-3 rounded-xl text-white outline-none focus:ring-2 focus:ring-purple-500 transition"
