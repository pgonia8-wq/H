import { useState, useEffect, useCallback, useRef } from "react";
  ...
  export default function TokenPage() {
    const { selectedTokenId, navigate, user } = useApp();
      ...
      <div className="min-h-full pb-24" data-testid="token-page">
        <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-2xl px-4 py-2.5 flex items-center gap-2.5 border-b border-border/20">
          ...
          <div className="text-right">
            <div className="text-sm font-black text-foreground font-mono">
              {token.priceWld < 0.001 ? token.priceWld.toExponential(2) : token.priceWld.toFixed(6)}
            </div>
        ...
        <div className="px-4 pt-3 space-y-3">
          ...
          <div className="grid grid-cols-4 gap-1.5">
            ...
            <Stat label="Vol 24h" value={`${formatCompact(token.volume24h)}`} />
            <Stat label="Liq" value={`${formatNum(token.totalWldInCurve ?? 0)}`} sub="WLD" />
            <Stat label="FDV" value={`${formatCompact(token.priceWld * token.totalSupply)}`} sub="WLD" />
          </div>
