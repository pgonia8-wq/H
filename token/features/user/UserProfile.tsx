...
export default function UserProfilePage() {
  const { user, balanceWld, balanceUsdc, walletAddress, navigate, openCreatorDashboard, fmtWld, displayCurrency, wldUsdRate } = useApp();
      ...
      <div className="px-4 pt-3 space-y-3">
        <div className="p-4 rounded-xl bg-gradient-to-br from-green-500/8 to-emerald-600/5 border border-green-500/15">
          ...
          <div className="text-2xl font-black text-foreground tracking-tight font-mono">
            {fmtWld(portfolioWld, { decimals: 4 })}
          </div>
          <div className={`flex items-center gap-1 mt-1 ${pnlPositive ? "text-green-400" : "text-red-400"}`}>
            ...
            <span className="text-xs font-bold">{pnlPositive ? "+" : ""}{fmtWld(totalPnl, { decimals: 4 })}</span>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="stat-card items-center">
            <span className="text-[9px] text-muted-foreground font-medium uppercase tracking-wider">Balance</span>
            <span className="text-sm font-bold text-foreground font-mono">{fmtWld(balanceWld, { decimals: 2 })}</span>
          </div>
          <div className="stat-card items-center">
                  ...
                  return (
                    <button key={h.tokenId} onClick={() => navigate("token", { tokenId: h.tokenId })}
                      ...
                      <div className="text-right shrink-0">
                        <div className="text-xs font-bold text-foreground font-mono">{fmtWld(h.value, { decimals: 4 })}</div>
