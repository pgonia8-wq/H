...
contract TotemGraduationManager is Ownable2Step, ReentrancyGuard, Pausable {
    ...
    // [C-02 FIX] Oracle y treasury necesarios para construir HumanTotem (6 params).
    ...
    // [C-02 FIX] Se añaden _oracle y _treasury — necesarios para HumanTotem(6 params).
    constructor(
        address _totem,
        address _curve,
        address _metrics,
    ...
    function _createAMM(address user, string memory name, string memory symbol) internal returns (address pair) {
        ...
        // [C-02 FIX] HumanTotem requiere 6 params: name, symbol, avatar, oracle, registry, treasury.
        // address(totem) implementa status() — compatible con ITotemRegistry de HumanTotem.
        HumanTotem newTotem = new HumanTotem(
            name,
            symbol,
            user,            // _totemAvatar
            oracle,          // _oracle
            address(totem),  // _registry (Totem.sol → status())
            treasury         // _treasury
        );
